...
class MySqliteHelper(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DB_VERSION) {
    ...
    companion object {
        ...
        // 创建签到数据库
        val CHECK_IN_DB_CREATE = "create table " + CHECK_IN_TABLE_NAME + "(" + CHECK_IN_CMD_ID + " integer primary key autoincrement, " + CHECK_IN_CMD_NAME + " text, " +
                CHECK_IN_CMD_PACKAGE + " text);"
        val CHECK_IN_CMD_DB_CREATE = "create table " + CHECK_IN_CMD_TABLE + "(" + CHECK_IN_CMD_ID + " text, " +
